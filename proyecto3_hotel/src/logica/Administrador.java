...
public class Administrador extends Empleado {
    ...
    public void generarReportesServicios() {
        // reporte de venta de cada producto del restaurante
        ArrayList<String> reporteServicios = new ArrayList<>();
        ArrayList<Integer> reporteServiciosCant = new ArrayList<>();
        ArrayList<String> reporteServiciosAnio = new ArrayList<>();
        try {
            BufferedReader br = new BufferedReader(new FileReader("../Proyecto3/proyecto3_hotel/data/base_reportes/servicios.txt"));
            String linea;
            linea = br.readLine();
            while (linea != null) {
                String[] partes = linea.split(";");
                ...
                reporteServicios.add(servicio);
                reporteServiciosCant.add(cantidad);
                reporteServiciosAnio.add(anio);
        ...
        // Create a dataset for the bar graph
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        for (int i=0; i<reporteServicios.size();i++) {
            if (reporteServiciosAnio.get(i).contains("2022")){
                dataset.addValue(reporteServiciosCant.get(i), reporteServicios.get(i), "2022");
            }
            else if (reporteServiciosAnio.get(i).contains("2023")){
                dataset.addValue(reporteServiciosCant.get(i), reporteServicios.get(i), "2023");
            }
            else if (reporteServiciosAnio.get(i).contains("2020")){
                dataset.addValue(reporteServiciosCant.get(i), reporteServicios.get(i), "2020");
            }
            else{
                dataset.addValue(reporteServiciosCant.get(i), reporteServicios.get(i), "2021");
            }
        ...
        // Create the bar chart
        JFreeChart chart = ChartFactory.createBarChart(
            "Reporte Servicios Utilizados por Años",     // Chart title
            "Años-Servicios",              // X-axis label
            "Cantidad",                 // Y-axis label
            dataset,                 // Dataset
            PlotOrientation.VERTICAL,// Plot orientation
            true,                    // Include legend
        ...
        try {
            // Create a PDF document
            Document document = new Document(PageSize.A4, 50, 50, 50, 50);
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("../Proyecto3/proyecto3_hotel/data/Reportes_PDF/ReporteServicios.pdf"));
            document.open();
    ...
    public void generarReportesTarifasHabitaciones() {
        // reporte de venta de cada producto del restaurante
        ArrayList<String> reportefacturas = new ArrayList<>();
        ArrayList<Integer> reportefacturas2 = new ArrayList<>();

        ArrayList<String> reporteSuite = new ArrayList<>();
        ArrayList<Integer> reporteSuite2 = new ArrayList<>();

        ArrayList<String> reporteSuiteDoble = new ArrayList<>();
        ArrayList<Integer> reporteSuiteDoble2 = new ArrayList<>();



        try {
            BufferedReader br = new BufferedReader(new FileReader("../Proyecto3/proyecto3_hotel/data/base_reportes/tarifas_estandar.txt"));
            String linea;
            linea = br.readLine();
            while (linea != null) {
                String[] partes = linea.split(";");
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader("../Proyecto3/proyecto3_hotel/data/base_reportes/tarifas_suite.txt"));
            String linea;
            linea = br.readLine();
            while (linea != null) {
                String[] partes = linea.split(";");
                String mes = partes[0];
                int precio = Integer.valueOf(partes[1]);
                reporteSuite.add(mes);
                reporteSuite2.add(precio);
                linea = br.readLine();
            }
            br.close();
        } catch (Exception e) {
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader("../Proyecto3/proyecto3_hotel/data/base_reportes/tarifas_suite2.txt"));
            String linea;
            linea = br.readLine();
            while (linea != null) {
                String[] partes = linea.split(";");
                String mes = partes[0];
                int precio = Integer.valueOf(partes[1]);
                reporteSuiteDoble.add(mes);
                reporteSuiteDoble2.add(precio);
                linea = br.readLine();
            }
            br.close();
        } catch (Exception e) {
        ...
        DefaultCategoryDataset dataset2 = new DefaultCategoryDataset();
        for (int i=0; i<reporteSuite.size();i++) {
            if (reporteSuite.get(i).contains("2022")){
                dataset2.addValue(reporteSuite2.get(i), "2022", reporteSuite.get(i).substring(0,2));
            }
            else if (reportefacturas.get(i).contains("2023")){
                dataset2.addValue(reporteSuite2.get(i), "2023", reporteSuite.get(i).substring(0,2));
            }
            else{
                dataset2.addValue(reporteSuite2.get(i), "2021", reporteSuite.get(i).substring(0,2));
            }
        ...
        DefaultCategoryDataset dataset3 = new DefaultCategoryDataset();
        for (int i=0; i<reporteSuiteDoble.size();i++) {
            if (reporteSuiteDoble.get(i).contains("2022")){
                dataset3.addValue(reporteSuiteDoble2.get(i), "2022", reporteSuiteDoble.get(i).substring(0,2));
            }
            else if (reportefacturas.get(i).contains("2023")){
                dataset3.addValue(reporteSuiteDoble2.get(i), "2023", reporteSuiteDoble.get(i).substring(0,2));
            }
            else{
                dataset3.addValue(reporteSuiteDoble2.get(i), "2021", reporteSuiteDoble.get(i).substring(0,2));
            }
           
        }
        // Create the bar chart
        JFreeChart chart = ChartFactory.createLineChart(
            "Reporte Valor Habitacion Estandar",     // Chart title
            "Meses",              // X-axis label
            "Valor en pesos",                 // Y-axis label
            dataset,                 // Dataset
            PlotOrientation.VERTICAL,// Plot orientation
        ...
        JFreeChart chart2 = ChartFactory.createLineChart(
            "Reporte Valor Habitacion Suite",     // Chart title
            "Meses",              // X-axis label
            "Valor en pesos",                 // Y-axis label
            dataset2,                 // Dataset
            PlotOrientation.VERTICAL,// Plot orientation
        ...
        JFreeChart chart3 = ChartFactory.createLineChart(
            "Reporte Valor Habitacion Suite Doble",     // Chart title
            "Meses",              // X-axis label
            "Valor en pesos",                 // Y-axis label
            dataset3,                 // Dataset
            PlotOrientation.VERTICAL,// Plot orientation
        ...
        try {
            // Create a PDF document
            Document document = new Document(PageSize.A4, 50, 50, 50, 50);
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("../Proyecto3/proyecto3_hotel/data/Reportes_PDF/ReporteTarifas.pdf"));
            document.open();
    ...
    public void generarReportesFacturas() {
        ...
        HashMap<String,Integer> reportefacturas2 = new HashMap<>();
        ...
        try {
            ...
            while (linea != null) {
                String[] partes = linea.split(";");
                String precio1 = partes[0];
                int precio = Integer.valueOf(partes[1]);
                reportefacturas2.put(precio1,precio);
                linea = br.readLine();
            }
            br.close();
        } catch (Exception e) {
        ...
        XYSeries series = new XYSeries("Precio por noche VS Gastos Restaurante");
        for (String k : reportefacturas2.keySet()) {
           
            series.add(Integer.valueOf(k), reportefacturas2.get(k));
        }
        ...
        JFreeChart chart = ChartFactory.createScatterPlot(
                "Relacion entre precio por noche y gastos restuarante", // Title
                "Precio Noches",        // X-axis label
                "Gastos",        // Y-axis label
                dataset,         // Dataset
        ...
        renderer.setSeriesShape(0, new java.awt.geom.Ellipse2D.Double(-3, -3, 6, 6));  // Set custom shape for data points
